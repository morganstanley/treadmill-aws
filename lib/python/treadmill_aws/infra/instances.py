...
class Instance(ec2object.EC2Object):
    ...
    def _reverse_dns_record_attrs(self):
        """Get reverse DNS record attributes."""
        return [
...
class Instances:
    ...
    def get(cls, ids=None, filters=None):
        """Get instances."""
        properties = Instances.load_json(ids=ids, filters=filters)
        return Instances(
            instances=[Instance(
    ...
    def _delete_host_from_ipa(self):
        """Delete hostname records from IPA."""
        _api = ipa.API()
        for _i in self.instances:
            if _i.role != constants.ROLES['IPA']:
                try:
                    ...
                    _LOGGER.warning(
                        'Unable delete host %s from IPA: %s',
                        _i.hostname,
                        err
                    )
    ...
    def _wait_for_termination(self):
        """Wait for termination of all instances."""
        if not self.ids:
            return

        def is_terminated(res):
            """Check if instance is terminated."""
            _LOGGER.info('Waiting for instances termination.')
