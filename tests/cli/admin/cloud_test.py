...
from treadmill_aws.infra import constants, connection
...
class CloudTest(unittest.TestCase):
    ...
    def setUp(self):
        self.patched = mock.patch(
            'treadmill_aws.infra.utils.cli_callbacks'
            '.pkg_resources.resource_string',  # noqa
        ...
        self.configure_cli = importlib.import_module(
            'treadmill_aws.cli.admin.cloud').init()

    def tearDown(self):
        self.patched.stop()

    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    ...
    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    def test_configure_vpc_dup_vpc_name(self, vpc_mock):
        """
        Test cloud configure vpc with duplicate vpc name.
        """
    ...
    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    @mock.patch('treadmill_aws.cli.admin.cloud.cell.Cell')
    @mock.patch('treadmill_aws.cli.admin.cloud.subnet.Subnet')
    ...
    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    @mock.patch('treadmill_aws.cli.admin.cloud.node.Node')
    @mock.patch('treadmill_aws.cli.admin.cloud.subnet.Subnet')
    def test_add_node(self, _subnet_mock, node_mock, vpc_mock):
        """
        Test add node.
        """
        vpc_mock.get_id_from_name = mock.Mock(return_value=self.vpc_id_mock)
        ...
        vpc_mock.get_id_from_name.assert_called_once_with(self.vpc_name)
        _node_mock.setup.assert_called_once_with(
    ...
    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    @mock.patch('treadmill_aws.cli.admin.cloud.ipa.IPA')
    @mock.patch('treadmill_aws.cli.admin.cloud.subnet.Subnet')
    ...
    def test_list_vpc(self, vpc_mock):
        ...
        self.assertEqual(result.exit_code, 0)
        self.assertEqual(vpc_mock.mock_calls[2],
                         mock.mock.call(instance_id=self.vpc_id_mock))
        _vpc_mock.show.assert_called_once()

    @unittest.skip('Something fishy with configure cell command.')
    @mock.patch('treadmill_aws.cli.admin.cloud.vpc.VPC')
    @mock.patch('treadmill_aws.cli.admin.cloud.subnet.Subnet')
    def test_list_cell(self, subnet_mock, vpc_mock):
        ...
        self.assertEqual(result.exit_code, 0)
        self.assertEqual(_vpc_mock.list_cells.call_count, 3)
